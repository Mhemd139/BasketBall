...
export function CreateTeamButton({ locale, canCreate }: { locale: string, canCreate: boolean }) {
    ...
    return (
        <>
            ...
            >
                <div className="w-8 h-8 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white/70 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-500 transition-all">
                    <Plus className="w-5 h-5" />
                </div>
                <span className="font-bold text-white/70 group-hover:text-indigo-300 transition-colors">
                    {'إضافة فريق جديد'}
                </span>
            </button>
