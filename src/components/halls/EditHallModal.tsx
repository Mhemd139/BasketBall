...
export function EditHallModal({ isOpen, onClose, hall, locale }: EditHallModalProps) {
    ...
    const [nameEn, setNameEn] = useState(hall?.name_en || '');
    ...
    const handleSave = async () => {
        setLoading(true);
        try {
            const res = await updateHall(hall.id, nameEn, nameAr, nameHe);
            if (res.success) {
                router.refresh();
                onClose();
            } else {
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <input 
                                    ...
                                    className="w-full p-4 rounded-xl border-2 border-gray-100 bg-white text-navy-900 font-bold outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100 transition-all shadow-sm group-hover:border-gray-200"
